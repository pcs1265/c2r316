...
# Load runtime.asm content at module load time
_RUNTIME_ASM_PATH = os.path.join(os.path.dirname(__file__), '..', 'runtime', 'runtime.asm')
try:
    with open(_RUNTIME_ASM_PATH, 'r', encoding='utf-8') as _f:
        _RUNTIME_ASM_CONTENT = _f.read()
except OSError:
    _RUNTIME_ASM_CONTENT = None  # Will be checked at runtime
...
class Codegen:
    ...
    def generate(self, prog: IRProgram) -> str:
        ...
        # Embed runtime.asm content directly
        if _RUNTIME_ASM_CONTENT is None:
            raise CodegenError("runtime/runtime.asm not found")
        for line in _RUNTIME_ASM_CONTENT.split('\n'):
            self._emit(line)
        self._emit('')
